...
def create_dataset(request):   
    ...
    while(True):
        ...
        for(x,y,w,h) in faces:
            ...
            cv2.imwrite('/Users/Roberto/Documents/Proyectos/Proyecto Final/Proyecto_Vet/ml/dataset/' +'usuario'+'.'+str(id)+'.'+str(sampleNum)+'.jpg', gray[y:y+h,x:x+w])
...
def trainer(request):
    ...
    #Path of the samples
    path = '/Users/Roberto/Documents/Proyectos/Proyecto Final/Proyecto_Vet/ml/dataset/'
    ...
    recognizer.save('/Users/Roberto/Documents/Proyectos/Proyecto Final/Proyecto_Vet/ml/recognizer/trainingData.yml')
    cv2.destroyAllWindows()
    success(request, 'Listo su Modelo a Sido Entrenado y esta listo para funcionar!!!')
...
def detect(request):
    ...
    while(True):
        ...
        for(x,y,w,h) in faces:
            ...
            if conf<35:
                userId = getId
                cv2.putText(img, "Identificado",(x,y+h), font, 2, (0,255,0),2)
            else:
                cv2.putText(img, "Desconocido",(x,y+h), font, 2, (0,0,255),2)
                t=t+1
                if t == 10:
                    cv2.waitKey(1000)
                    cam.release()
                    cv2.destroyAllWindows()
                    request.session['form_message'] = "Rostro Desconocido, por favor cree una cuenta"
